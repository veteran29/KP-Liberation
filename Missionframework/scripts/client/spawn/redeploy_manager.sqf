...
// Function which fill the list and handles list item selection
KPLIB_redeploy_listHandlerLoop = {
...
KPLIB_redeploy_handleRespawn = {
	...
	if (KP_liberation_arsenalUsePreset) then {
		[(backpack player)] call F_checkGear;
	};
...
private _redeploy_mainLoop = {
	private _loopCondition = (GRLIB_force_redeploy || (player distance (getmarkerpos GRLIB_respawn_marker) < 50)) && vehicle player == player && alive player && !dialog && howtoplay == 0;
	// Exit when condition not met
	if(!_loopCondition) exitWith {};
	...
	// Wait until dialog is open and execute rest of the code then
	[{dialog}, {
		fullmap = 0;
		deploy = 0;
		...
		[
			...
			1, 	// delay between runs
			[], // args
			{
				// start code, init vars
				_oldsel = -999;
				_old_fullmap = 0;
...
// Start the loop
KPLIB_redeploy_PFH = [_redeploy_mainLoop, 0.5] call CBA_fnc_addPerFrameHandler;
